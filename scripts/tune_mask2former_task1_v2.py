#!/usr/bin/env python3
"""Error-driven, class-specific Task-1 tuning for Mask2Former Swin-Tiny.

Rules are selected using only the outer-fold validation case. The two test
cases are loaded only after calibration is frozen. Detailed class/image/
component reports are emitted to make every accepted rule auditable.
"""
...
VERSION = "mask2former_swin_tiny_task1_tuned_v2_error_driven_r1"
...
def task1_score(fine_dice: float, fine_nhd: float) -> float:
    return float((fine_dice + 1.0 - fine_nhd) / 2.0)


def prepare_prediction(semantic: torch.Tensor) -> dict:
    values = semantic.float().clamp_min(0)
    probabilities = (values / values.sum(0, keepdim=True).clamp_min(1e-8)).cpu().numpy()
    base = probabilities.argmax(0).astype(np.uint8)
    top_two = np.argpartition(probabilities, kth=-2, axis=0)[-2:]
    second = np.where(top_two[0] == base, top_two[1], top_two[0]).astype(np.uint8)
    flat_probability = probabilities.reshape(probabilities.shape[0], -1)
    flat_second = second.ravel()
    components: list[dict] = []
    for class_id in range(1, probabilities.shape[0]):
...
def apply_rules(prepared: dict, rules: dict[int, dict]) -> np.ndarray:
    output = prepared["base"].copy()
    flat_output = output.ravel()
    flat_second = prepared["second"].ravel()
    for component in prepared["components"]:
        rule = rules.get(component["class_id"])
        if rule is None:
            continue
        if (component["area"] < rule["minimum_area"]
                and component["mean_confidence"] < rule["minimum_confidence"]):
            indices = component["flat_indices"]
            if rule["replacement"] == "background":
                flat_output[indices] = 0
            else:
...
def infer_split(model, config: dict, data_root: Path, labelmap: LabelMap, fold: int,
    ...
    for batch in loader:
        ...
        prepared.append(prepare_prediction(semantic))
        targets.append(batch["fine"].numpy()[0])
        cases.append(batch["case_id"][0])
        names.append(batch["name"][0])
    return prepared, targets, cases, names


def metric_arrays(predictions: list[np.ndarray], targets: list[np.ndarray], classes: int) -> tuple[np.ndarray, np.ndarray]:
    dices = np.empty((len(predictions), classes), dtype=np.float64)
    nhds = np.empty_like(dices)
    for image_index, (prediction, target) in enumerate(zip(predictions, targets)):
        for class_id in range(classes):
            ...
            dices[image_index, class_id] = binary_dice(pred_binary, target_binary)
            nhds[image_index, class_id] = normalized_hausdorff(pred_binary, target_binary)
    return dices, nhds


def update_metric_arrays(previous_predictions: list[np.ndarray], predictions: list[np.ndarray],
                         targets: list[np.ndarray], previous: tuple[np.ndarray, np.ndarray]) -> tuple[np.ndarray, np.ndarray]:
    """Recompute only classes touched by a candidate rule."""
    dices, nhds = previous[0].copy(), previous[1].copy()
    for image_index, (old, new, target) in enumerate(zip(previous_predictions, predictions, targets)):
        changed = old != new
        if not changed.any():
            continue
        class_ids = np.union1d(old[changed], new[changed])
        for class_id_value in class_ids:
            class_id = int(class_id_value)
            pred_binary = new == class_id
            target_binary = target == class_id
            dices[image_index, class_id] = binary_dice(pred_binary, target_binary)
            nhds[image_index, class_id] = normalized_hausdorff(pred_binary, target_binary)
    return dices, nhds
...
def summarize_arrays(dices: np.ndarray, nhds: np.ndarray, weights: np.ndarray) -> dict:
    image_dice = dices @ weights / weights.sum()
    image_nhd = nhds @ weights / weights.sum()
    image_score = (image_dice + 1.0 - image_nhd) / 2.0
    return {
        "fine_dice": float(image_dice.mean()),
        "fine_nhd": float(image_nhd.mean()),
        "task1_score": float(image_score.mean()),
        "per_image_score": image_score,
    }


def complete_misses(predictions: list[np.ndarray], targets: list[np.ndarray], weights: np.ndarray) -> int:
    total = 0
    protected = np.flatnonzero(weights >= 3)
    for prediction, target in zip(predictions, targets):
        total += sum(bool((target == class_id).any()) and not bool((prediction == class_id).any())
                     for class_id in protected)
    return int(total)
...
def class_and_image_rows(fold: int, split: str, stage: str, predictions: list[np.ndarray],
    ...
    for prediction, target, case, name in zip(predictions, targets, cases, names):
        for class_id, class_name in enumerate(labelmap.fine_names):
            ...
            image_rows.append({
                ...
                "weight": float(labelmap.fine_weights[class_id]),
    ...
    for class_id, class_name in enumerate(labelmap.fine_names):
        ...
        class_rows.append({
            ...
            "class_name": class_name, "weight": float(labelmap.fine_weights[class_id]),
            ...
            "absent_fp_rate": (sum(row["absent_false_positive"] for row in selected) / absent
                               if absent else 0.0),
            "complete_miss_count": sum(row["complete_miss"] for row in selected),
            "complete_miss_rate": (sum(row["complete_miss"] for row in selected) / present
                                   if present else 0.0),
            "error_priority": float(labelmap.fine_weights[class_id] * occurrence
                                    * ((1.0 - dice) + nhd)),
        })
    ...
    for item, target, case, name in zip(prepared, targets, cases, names):
        ...
        for component in item["components"]:
            ...
            rows.append({
                ...
                "class_id": class_id, "class_name": labelmap.fine_names[class_id],
                "weight": float(labelmap.fine_weights[class_id]),
...
def confusion_rows(fold: int, split: str, stage: str, predictions: list[np.ndarray],
    ...
    matrix = np.zeros((31, 31), dtype=np.int64)
    for prediction, target in zip(predictions, targets):
        matrix += np.bincount(target.ravel() * 31 + prediction.ravel(), minlength=31 * 31).reshape(31, 31)
    rows = []
    for true_id in range(31):
        for pred_id in range(31):
            if matrix[true_id, pred_id]:
                rows.append({
                    "fold": fold, "split": split, "stage": stage,
                    "true_class_id": true_id, "true_class_name": labelmap.fine_names[true_id],
                    "pred_class_id": pred_id, "pred_class_name": labelmap.fine_names[pred_id],
...
def tune_rules(prepared: list[dict], targets: list[np.ndarray], class_rows: list[dict],
    ...
    current_arrays = metric_arrays(current_predictions, targets, len(labelmap.fine_names))
    current_summary = summarize_arrays(*current_arrays, labelmap.fine_weights)
    current_misses = complete_misses(current_predictions, targets, labelmap.fine_weights)
    eligible = sorted(
    ...
    for step in range(args.max_rules):
        ...
        for class_row in eligible:
            ...
            for parameters in candidate_parameters:
                ...
                summary = summarize_arrays(*arrays, labelmap.fine_weights)
                ...
                misses = complete_misses(predictions, targets, labelmap.fine_weights)
                row = {
                    "step": step, "class_id": class_id,
                    "class_name": class_row["class_name"], **parameters,
                    "fine_dice": summary["fine_dice"], "fine_nhd": summary["fine_nhd"],
                    "task1_score": summary["task1_score"],
                ...
                search.append(row)
                if row["eligible"] and (best is None or row["task1_score"] > best[0]["task1_score"]):
...
def exact_metrics(predictions: list[np.ndarray], targets: list[np.ndarray], cases: list[str],
    ...
    details = segmentation_metrics(predictions, targets, cases,
                                   labelmap.fine_weights, labelmap.fine_names)
    accuracy = float(np.mean([(prediction == target).mean()
                              for prediction, target in zip(predictions, targets)]))
    return {
        "fine_dice": details["dice"], "fine_nhd": details["nhd"],
        "fine_pixel_accuracy": accuracy,
        "task1_score": task1_score(details["dice"], details["nhd"]),
    }
...
def run_fold(args: argparse.Namespace, config: dict, labelmap: LabelMap,
    ...
    validation = infer_split(model, config, args.data_root, labelmap, fold, "validation", device)
    val_prepared, val_targets, val_cases, val_names = validation
    ...
    calibration["validation_gain"] = (
        calibration["tuned"]["task1_score"] - calibration["baseline"]["task1_score"]
    )
    (fold_dir / "calibration.json").write_text(json.dumps(calibration, indent=2) + "\n")
    write_csv(fold_dir / "validation_rule_search.csv", search)

    # Test is loaded only after validation calibration is frozen above.
    test = infer_split(model, config, args.data_root, labelmap, fold, "test", device)
    test_prepared, test_targets, test_cases, test_names = test
    ...
    prediction_dir = fold_dir / "test_predictions_ids"
    ...
    result = {
        ...
        "test_gain": tuned_metrics["task1_score"] - baseline_metrics["task1_score"],
    }
...
def main() -> None:
    ...
    if not torch.cuda.is_available():
        raise RuntimeError("Task-1 v2 experiment requires a CUDA node")
    ...
    metadata = {
        ...
        "tuning_scope": "Task 1 error-driven class-specific post-processing",
    ...
    keys = ("fine_dice", "fine_nhd", "fine_pixel_accuracy", "task1_score")
    summary = {
    ...
    selected_classes = Counter(
        class_id for row in folds for class_id in row["selected_rules"]
    )
    output = {
        **metadata, "completed_test_folds": len(folds), "folds": folds,
        ...
        "acceptance": {
            "mean_task1_score_improved": bool(gains.mean() > 0),
