#!/usr/bin/env python3
"""Tune Task-1 post-processing for Mask2Former Swin-Tiny without test leakage.

Each outer fold tunes one conservative connected-component rule on that fold's
single validation case, freezes it, and evaluates the two held-out test cases.
The source checkpoints are read-only; all outputs go to a versioned directory.
"""
...
VERSION = "mask2former_swin_tiny_task1_tuned_v1"
...
def task1_score(metrics: dict) -> float:
    return float((metrics["fine_dice"] + 1.0 - metrics["fine_nhd"]) / 2.0)
...
def prepare_hard_prediction(semantic: torch.Tensor, class_weights: np.ndarray) -> dict:
    values = semantic.float().clamp_min(0)
    probabilities = values / values.sum(0, keepdim=True).clamp_min(1e-8)
    probability = probabilities.cpu().numpy()
    base = probability.argmax(0).astype(np.uint8)
    top_two = np.argpartition(probability, kth=-2, axis=0)[-2:]
    ...
    for class_id in range(1, probability.shape[0]):
        ...
        for component_id in range(1, count + 1):
            ...
            components.append({
                "class_id": class_id,
                "weight": float(class_weights[class_id]),
                "area": int(sizes[component_id]),
                "mean_confidence": float(probability[class_id].ravel()[flat_indices].mean()),
...
def apply_rule(prepared: dict, minimum_area: int, minimum_confidence: float) -> np.ndarray:
    ...
    for component in prepared["components"]:
        # Clinical-weight 3 structures receive a 3x smaller area threshold.
        class_area_threshold = max(1, math.ceil(minimum_area / component["weight"]))
        remove = (
            component["area"] < class_area_threshold
...
def fine_metrics(predictions: list[np.ndarray], targets: list[np.ndarray], cases: list[str],
                 labelmap: LabelMap) -> dict:
    details = segmentation_metrics(
        predictions, targets, cases, labelmap.fine_weights, labelmap.fine_names,
    )
    pixel_accuracy = float(np.mean([
        (prediction == target).mean() for prediction, target in zip(predictions, targets)
    ]))
    result = {
        "fine_dice": details["dice"],
        "fine_nhd": details["nhd"],
        "fine_pixel_accuracy": pixel_accuracy,
    }
    result["task1_score"] = task1_score(result)
    return result
...
def infer_split(model, config: dict, data_root: Path, labelmap: LabelMap, fold: int,
    ...
    for batch in loader:
        ...
        prepared.append(prepare_hard_prediction(semantic, labelmap.fine_weights))
...
def tune_rule(prepared: list[dict], targets: list[np.ndarray], cases: list[str],
    ...
    for candidate in candidates:
        ...
        metrics = fine_metrics(predictions, targets, cases, labelmap)
        search.append({**candidate, **metrics})
    baseline = search[0]
    best = max(search, key=lambda row: (row["task1_score"], -row["minimum_area"], -row["minimum_confidence"]))
    if best["task1_score"] < baseline["task1_score"] + minimum_gain:
        best = baseline
    selected = {
        "minimum_area": int(best["minimum_area"]),
        "minimum_confidence": float(best["minimum_confidence"]),
        "validation_gain": float(best["task1_score"] - baseline["task1_score"]),
    }
...
def run_fold(args: argparse.Namespace, config: dict, labelmap: LabelMap,
    ...
    tuned_predictions = [apply_rule(item, selected["minimum_area"], selected["minimum_confidence"])
                         for item in test_prepared]
    baseline_metrics = fine_metrics(baseline_predictions, test_targets, test_cases, labelmap)
    tuned_metrics = fine_metrics(tuned_predictions, test_targets, test_cases, labelmap)
...
def main() -> None:
    ...
    if args.output_dir.resolve() == args.model_dir.resolve():
        raise ValueError("Tuned output directory must differ from the source model directory")
    if args.output_dir.exists() and any(args.output_dir.iterdir()):
        raise FileExistsError(f"Refusing to overwrite non-empty tuned version: {args.output_dir}")
    if not torch.cuda.is_available():
        raise RuntimeError("Task-1 tuning must run on a CUDA node")
    ...
    summary_keys = ("fine_dice", "fine_nhd", "fine_pixel_accuracy", "task1_score")
    summary = {
        stage: {key: metric_summary([row[stage] for row in folds], key) for key in summary_keys}
        for stage in ("baseline", "tuned")
    }
    summary["task1_score_gain"] = metric_summary(folds, "test_gain")
    output = {
        **metadata,
        ...
        "summary": summary,
        "acceptance": {
            "mean_task1_score_improved": (
                summary["tuned"]["task1_score"]["mean"]
                > summary["baseline"]["task1_score"]["mean"]
            ),
