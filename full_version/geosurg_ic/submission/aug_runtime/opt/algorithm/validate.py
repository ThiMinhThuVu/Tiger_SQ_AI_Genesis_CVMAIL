"""Validate standalone Task 2 output."""
import argparse
...
from model import TASK2_PALETTE
...
def _validate_mask(path: Path, expected_size: tuple[int, int]) -> None:
    with Image.open(path) as handle:
        if handle.format != "PNG" or handle.mode != "RGB":
            raise ValueError(f"Task 2 mask must be an RGB PNG: {path}")
        if handle.size != expected_size:
            raise ValueError(
                f"Mask size {handle.size} differs from input {expected_size}: {path}"
            )
        array = np.asarray(handle)
    allowed = set(int(value) for value in _packed(TASK2_PALETTE.reshape(-1, 1, 3)).reshape(-1))
...
def validate_output(input_dir: Path, output_dir: Path) -> None:
    ...
    actual = {path.name for path in (output_dir / "task2").glob("*.png")}
    if not inputs or actual != expected:
        raise ValueError(f"Task 2 filenames differ: missing={sorted(expected-actual)[:5]}, extra={sorted(actual-expected)[:5]}")
    for input_path in inputs:
        with Image.open(input_path) as image:
            size = image.size
        _validate_mask(output_dir / "task2" / input_path.name, size)
    extras = {path.name for path in output_dir.iterdir()} - {"task2"}
    if extras:
